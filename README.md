...
### 💎 چرا این نسخه تغییر شگرفی ایجاد می‌کند؟
1. **جذابیت بصری بالا:** داشتن دکمه‌ها و Badgeهای رسمی در بالای صفحه بلافاصله حس یک پروژه جدی و نگهداری‌شده (Maintained) را القا می‌کند.
2. **برتری فنی واضح:** جدول مدل‌های پیشنهادی و جزییات VRAM، داکیومنت شما را از یک «پروژه تمرینی» به یک «پایپ‌لاین مهندسی‌شده» تبدیل می‌کند.
3. **وضوح بالا در گام‌های نصب:** استفاده از کامنت در کدهای Bash و نمایش قطعه کد فرضی از فایل `config.py` به کاربر نشان می‌دهد دقیقاً کجای کار را باید ویرایش کند تا برنامه استارت بخورد.
```